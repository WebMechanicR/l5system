...
double TL_price = 0;
if(TL_1_starttime >= TL_2_starttime && TL_1_starttime >= TL_3_starttime && TL_1_starttime > TL_4_starttime){
   ...
   TL_price = TL_1_2_price;
}
if(TL_2_starttime >= TL_1_starttime && TL_2_starttime >= TL_3_starttime && TL_2_starttime > TL_4_starttime){
   if (TL_2_2_price > ObjectGetValueByShift("TL_2_2", 5)) TrendUp=true;
   else if (TL_2_2_price < ObjectGetValueByShift("TL_2_2", 5)) TrendDown=true;
   TL_price = TL_2_2_price;
}
if(TL_3_starttime >= TL_1_starttime && TL_3_starttime >= TL_2_starttime && TL_3_starttime > TL_4_starttime){
   if (TL_3_2_price > ObjectGetValueByShift("TL_3_2", 5)) TrendUp=true;
   else if (TL_3_2_price < ObjectGetValueByShift("TL_3_2", 5)) TrendDown=true;
   TL_price = TL_3_2_price;
}
if(TL_4_starttime >= TL_1_starttime && TL_4_starttime >= TL_2_starttime && TL_4_starttime > TL_3_starttime){
   if (TL_4_2_price > ObjectGetValueByShift("TL_4_2", 5)) TrendUp=true;
   else if (TL_4_2_price < ObjectGetValueByShift("TL_4_2", 5)) TrendDown=true;
   TL_price = TL_4_2_price;
}
   ...
   if(TrendUp || TrendDown){
      ...
      if(Digits == 3)
         p = 0.01;
      double pips = p * distance;
      double diffr = TL_price - Ask;
      if(TrendDown && diffr < -distance){
         signalBuffer[0] = 1;
      }
      else if(TrendDown && MathAbs(diffr) < distance){
         signalBuffer[0] = -1;
      }
      else if(TrendUp && diffr > distance){
         signalBuffer[0] = -1;
      }
      else if(TrendUp && MathAbs(diffr) < distance){
         signalBuffer[0] = 1;
      }
...
if (timeDiff != Server_Local_TimeDiff) {
   Server_Local_TimeDiff = timeDiff;
}
