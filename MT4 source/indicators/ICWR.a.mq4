...
{                                      // from the chart upon removal
                                       // of the indicator
   ObjectDelete("ActiveWaveICWR");
   ObjectDelete("FiboICWR");
   return(0);
}
      ...
      if(ExtMapBuffer[shift]!=0.0)
      {
         if(LastActive > 0)
         {
            if(MathAbs(LastActive - ExtMapBuffer[shift]) >= RequiredWaveHeight*Point)
            {
               ...
               if(ObjectFind("ActiveWaveICWR") != 0)
               {
                  ObjectCreate("ActiveWaveICWR", OBJ_TREND, 0, Time[AWStartPos], AWStart, Time[AWEndPos], AWEnd);
                  ObjectSet("ActiveWaveICWR", OBJPROP_STYLE, STYLE_SOLID);
                  ObjectSet("ActiveWaveICWR", OBJPROP_COLOR, Red);               
                  ObjectSet("ActiveWaveICWR", OBJPROP_WIDTH, 3);
                  ObjectSet("ActiveWaveICWR", OBJPROP_RAY, false);  
               }
               else
               {
                  ObjectMove("ActiveWaveICWR", 0, Time[AWStartPos], AWStart);
                  ObjectMove("ActiveWaveICWR", 1, Time[AWEndPos], AWEnd);
               }
               ...
               if(ObjectFind("FiboICWR") != 0)
               {
                  ObjectCreate("FiboICWR", OBJ_FIBO, 0, Time[AWStartPos], AWStart, Time[AWEndPos], AWEnd);
                  ObjectSet("FiboICWR", OBJPROP_STYLE, STYLE_SOLID);
                  ObjectSet("FiboICWR", OBJPROP_COLOR, Green);               
                  ObjectSet("FiboICWR", OBJPROP_WIDTH, 2);
                  ObjectSet("FiboICWR", OBJPROP_FIBOLEVELS, 6);
/*                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+0, FiboICWRL);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+1, FiboICWRLC);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+2, FiboICWRLR);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+3, FiboICWRHR);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+4, FiboICWRHC);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+6, FiboICWRH); */
                  
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+0, 0);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+1, 0.25);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+2, 0.382);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+3, 0.618);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+4, 0.75);
                  ObjectSet("FiboICWR", OBJPROP_FIRSTLEVEL+5, 1);
               }
               else
               {
                  ObjectMove("FiboICWR", 0, Time[AWStartPos], AWStart);
                  ObjectMove("FiboICWR", 1, Time[AWEndPos], AWEnd);
               }
               break;
            }
         }
